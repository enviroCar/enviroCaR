...
require(enviroCaR)
tracksCol <- importEnviroCar(baseurl, "5207d871e4b058cd3d669afe")
@
...
stplot(tracksCol, attr = "speed", lwd = 3, scales = list(draw = T))
@
...
stplot(tracksCol, scales = list(draw = TRUE), sp.layout = sp.raster, 
    col = "red", lwd = 2, main = "google map background")
@

<<>>=
sp <- as(tracksCol, "SpatialPointsDataFrame")
spplot(sp, "Consumption", colorkey=T)
@
...
<<>>=
spplot(sp.agg[c("Rpm", "Speed", "CO2", "Consumption")], colorkey = TRUE, sp.layout = list("sp.points", sp, 
